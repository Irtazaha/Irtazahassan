...
\033[1;91m            ###   ###    ##   ##   ##  ###   ## ##    ##\033[1;0m
\033[1;97m           #### ####    ##  ##    ##  ####  ## ##\033[1;0m
\033[1;97m        ZOIIIIWW.COM .FEEL THE POWER OF ZOIIW 
\033[1;91m        
\033[1;91m       
\033[1;97m    
\033[1;97m--------------------------------------------------
\033[1;91m Author      : zoiiw ignore karne wali 
\033[1;91m GitHub      : https://github.com/Mohammadjan1122
\033[1;91m YouTube     : nhi banaya 
\033[1;91m Telegram    : aman khan
...
def menu():
    ...
    jid = ''
    try:
        _zoiiwvau_ = _zoiiwvau_dapunta_("token.txt","r").read();_cici_ = _azimvau_dapunta_("cookies.txt","r").read();_salsabila_ = {"cookie" : _cici_}
    ...
    try:token = _zoiiwvau_dapunta_("token.txt","r").read();x = _req_get_("https://graph.facebook.com/me?access_token=" + token);y = _js_lo_(x.text);n = y['name'].upper();i = y['id']
...
def defaultua():
    ua = ua_xiaomi
    try:ugent = _zoiiwvau_dapunta_('ugent.txt','w');ugent.write(ua);ugent.close()
    except (KeyError,IOError):menu_log()


def ugen():
    var_ugen()
    pmu = _cici_zoiiwvau_('%s [%s•%s] %sCHOOSE : '%(H,P,H,K))
    print('%s '%(O))
